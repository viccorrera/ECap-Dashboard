...
    (
        (
            WITH INRE_COM_DATA_NEW AS -- Extract of commercial exposure and ECAP aggregated at parent level
            (
                SELECT
...
            ORDER BY
                EXPOSURE_NEW DESC -- Ordering resulting Exposure by descending order which will allow us to use the function ROWNUM to extract the top 40 rows (see last line of query)
        )
        LEFT JOIN -- Joining in data from previous position
        (
...
                    SUM(CREDIT_LIMIT_NET_EXPOSURE * ULTIMATE_POD) / SUM(CREDIT_LIMIT_NET_EXPOSURE) AS PD
                FROM
                    CALC6673.SO_REPORTING d --This should correspond to the old schema
                WHERE
                    model_type = 'IR'
                    AND MODEL_SUB_TYPE IN ('CI_COM_KN', 'BO_COM_KN')
                GROUP BY
